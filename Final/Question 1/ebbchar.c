...
static ssize_t dev_write(struct file *filep, const char *buffer, size_t len, loff_t *offset){
   ...
   message[len]='\0';
   printk(KERN_INFO "EBBChar: %s \n", message);
   while(*message=='P'){
   printk("Die gewahlte Form ist [ Dreieckiges Prisma ]- (P)");
   printk("Grundflache und Volumen werden in Kurze berechnet");
   break;
   }
   while(*message=='K'){
   printk("Die gewahlte Form ist [ Kegel ]- (K)");
   printk("Grundflache und Volumen werden in Kurze berechnet");
   break;
   }
