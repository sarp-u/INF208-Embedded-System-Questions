...
static ssize_t dev_write(struct file *filep, const char *buffer, size_t len, loff_t *offset){
	...
	char* kbuffer;
   int gecici;
   sscanf(message, "%d", &gecici);
   if(gecici>=90 && gecici<=100){
      printk(KERN_INFO "AA");
   } 
   if(gecici>=85 && gecici<=89){
      printk(KERN_INFO "BA");
   } 
   if(gecici>=80 && gecici<=84){
      printk(KERN_INFO "BB");
   } 
   if(gecici>=75 && gecici<=79){
      printk(KERN_INFO "CB");
   } 
   if(gecici>=65 && gecici<=74){
      printk(KERN_INFO "CC");
   } 
   if(gecici>=58 && gecici<=64){
      printk(KERN_INFO "DC");
   } 
   if(gecici>=50 && gecici<=57){
      printk(KERN_INFO "DD");
   } 
   if(gecici>=40 && gecici<=49){
      printk(KERN_INFO "FD");
   }
   if(gecici<=39){
      printk(KERN_INFO "FF");
	...
	printk(KERN_INFO "EBBChar: Received %.2d\n", gecici);
	kfree(kbuffer);
	return len;
}
